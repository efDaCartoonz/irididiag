...
# Standalone iRidi Pro cloud diagnostic for the EU region.
# Usage: sh check_iridi_pro_eu.sh
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/iridi-pro-eu.XXXXXX" 2>/dev/null)"
if [ -z "$WORK_DIR" ] || [ ! -d "$WORK_DIR" ]; then
  WORK_DIR="${TMPDIR:-/tmp}/iridi-pro-eu.$$"
...
check_gate() {
  ...
  separator
  printf 'Cloud Gate EU: %s, порты 9088/9089\n' "$GATE_HOST"
  if [ -n "$GATE_LINE" ]; then
...
printf 'iRidi Pro Cloud Check — регион %s\n' "$REGION"
...
probe_resource auth-eu "Авторизация EU" "https://auth.eu.iridi.com/" "95.216.162.71" yes
probe_resource i3pro-eu "Облако i3 Pro EU" "https://i3pro.eu.iridi.com/" "95.216.162.71" yes
probe_resource storage-eu "Хранилище AWS EU" "https://s3.us-east-1.amazonaws.com/" "динамический" yes
probe_resource projects-eu "Проекты i3 Pro EU" "https://iridium-cloud-files.s3.amazonaws.com/" "динамический" yes
probe_resource updates-site "Сервис обновлений" "http://iridi.com/" "89.169.183.139" no
probe_resource updates-s3 "Файлы обновлений" "http://iridium3download.s3.amazonaws.com/" "динамический" no
check_gate
